...
function load_mailbox(mailbox, query = "") {
    ...
    document.querySelector(
        "#emails-view"
    ).innerHTML = `<h4 class="mailbox_head py-2 pl-3 m-0 mx-1">${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)
    }</h4> ${spinner}`;
        ...
        .then((emails) => {
          ...
          ${(() => {
              ...
              <div class="subject text-left">
                <div class="d-inline">${email.subject.length > 0 ? email.subject : "(no subject)"
                                }</div>
